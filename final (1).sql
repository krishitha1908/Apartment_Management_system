...
--DROP TRIGGER Tenant_Audit_Trigger;

-- Create the audit trigger again
CREATE OR REPLACE TRIGGER Tenant_Audit_Trigger
...
BEGIN
    IF INSERTING THEN
        ...
        VALUES (:NEW.house_no, :NEW.tenant_name, :NEW.phone_number, :NEW.apartment_id, :NEW.move_in_date, 'INSERT');
    ELSIF UPDATING THEN
        INSERT INTO Audit_Tenant (house_no, tenant_name, phone_number, apartment_id, move_in_date, action)
        VALUES (:NEW.house_no, :NEW.tenant_name, :NEW.phone_number, :NEW.apartment_id, :NEW.move_in_date, 'UPDATE');
    ELSIF DELETING THEN
        INSERT INTO Audit_Tenant (house_no, tenant_name, phone_number, apartment_id, move_in_date, action)
        VALUES (:OLD.house_no, :OLD.tenant_name, :OLD.phone_number, :OLD.apartment_id, :OLD.move_in_date, 'DELETE');
    END IF;
END;
/

CREATE OR REPLACE PROCEDURE update_due_amount_proc (
    p_house_no IN SYSTEM.Tenant.house_no%TYPE,
    p_increment IN NUMBER
) 
IS
BEGIN
    UPDATE SYSTEM.Tenant 
    SET due_amount = due_amount + p_increment
    WHERE house_no = p_house_no;
END;
/
